...
def update_profile(
    ...
    the tool's return message for exactly what was rejected and why):
      ...
      plan_mode: "full" (workout+diet) OR "diet_only" (meals/nutrition only).
            ...
            height_cm and weight_kg are ALWAYS required (for BMR calories).
...
def generate_plan() -> str:
    ...
    if profile.plan_mode == "diet_only":
        ...
        return (
            f"Diet-only plan generated{warn} (no workout days). Summarize the "
    ...
    return (
        f"Plan generated successfully ({n_ex} distinct exercises) and held in session.{warn} "
...
AGE & GENDER & SIZE: age + sex + height_cm + weight_kg are required before
generate_plan (Mifflin–St Jeor BMR). Ask for height/weight if missing.
They soft-bias advice; health flags and equipment drive hard exercise filters.
